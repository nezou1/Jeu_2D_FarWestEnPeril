    ...
    private void ajouterEnnemisParType(Random rand, int nombreEnnemis,String type, int minY, int maxY, int minX, int maxX) {
        ...
        for (int i = 0; i < nombreEnnemis; i++) {
            ...
            if (type.equals("Cowboy") ) {
                ennemi = new Cowboy(environnement.getTerrain(), environnement);
            } else {
                ennemi = new Dragon(environnement.getTerrain(), environnement);
            }
            placerEnnemiAleatoirement(rand, ennemi, environnement.getTerrain(), minY, maxY, minX, maxX);
            ennemis.add(ennemi);
